...
class NowPaymentsXService {
    ...
    public function __construct() {
        ...
        self::$apiKey = env('NAK') ?? config('services.nowpayments.api_key');
        self::$ipnSecret = env('NIK') ?? config('services.nowpayments.ipn_secret');
    }
    ...
    public static function createInvoice($deposit) {
        $payload = [
            ...
            'ipn_callback_url' => route('webhookx.nowpayments'),
            'order_description' => $deposit->narration . ' | creaded by user - ' . Auth::id()
        ];
    ...
    public static function verifySignature($rawPayload, $receivedSignature) {
        ...
        $expected = hash_hmac('sha512', $rawPayload, env('NIK'));
