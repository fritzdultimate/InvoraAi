<div x-data="{deletingId: @entangle('deletingId')}">
    <!-- Sidebar start -->
    <div class="col-span-12 xl:col-span-4">
        <div class="card border-0 rounded-2xl">
            <div class="card-header">
                <div class="flex items-center flex-wrap gap-2 justify-between">
                    <h6 class="font-bold text-lg mb-0">Fund Account</h6>
                </div>
            </div>
            <div class="card-body p-0">
                <div class="p-5">
                    <div class="relative z-[1] py-8 text-center px-4">
                        ...
                        <span class="text-white">Deposit Balance</span>
                    </div>
                </div>
                ...
                <div class="py-4 px-6">
                    ...
                    <form wire:submit.prevent="makeDeposit">
                            ...
                            <a data-modal-target="default-modal" data-modal-toggle="default-modal"
                                ...
                                <span class="flex items-center gap-4">
                                    ...
                                    <span
                                        ...
                                        {{ $selectedWallet['label'] ?? 'Select Currency' }}
                                    </span>
                                </span>
                        ...
                        <div class="mt-4">
                            <label for="message" class="block font-semibold text-neutral-600 dark:text-white mb-2">
                                Short narration
                            </label>
                            <textarea wire:model="narration" class="form-control" id="message" rows="2" cols="50"
                        ...
                        <div class="mt-4">
                            <button class="btn btn-primary-600 flex-shrink-0 flex items-center gap-2 px-8"
                                type="submit">
                                <div wire:target="makeDeposit" wire:loading.remove>
                                    Send
                                    <i class="ri-send-plane-fill"></i>
                                </div>

                                <x-btn-loader wire:target="makeDeposit" wire:loading />
    ...
    <div class="card border-0 rounded-2xl mt-6">
        <div class="card-header">
            <div class="flex items-center flex-wrap gap-2 justify-between">
                <h6 class="font-bold text-lg mb-0">Deposit History</h6>
        ...
        <div class="card-body">
            <div class="overflow-x-auto">
                <div class="w-[100px]">
                    @if(!$deposits->count())
                        <p class="w-full">No deposit history at the moment.</p>
                    @else
                            ...
                            <tbody>
                                @foreach ($deposits as $deposit)
                                    <tr class="text-xs md:text-sm">
                                        ...
                                        <td class="text-center font-semibold">${{ number_format($deposit->amount, 2) }}</td>
                                        <td class="text-center">{{ $deposit->created_at->format('d M Y - h:i A') }}</td>
                                        <td class="text-center">
                                            @php

                                            @endphp
                                            <span
                                                class="bg-{{ $deposit->status->color() }}-focus text-{{ $deposit->status->color() }}-main dark:text-{{ $deposit->status->color() }}-500 px-6 py-1 rounded-[50rem] font-medium text-sm capitalize">
                                                {{ $deposit->status->value }}
                                            </span>
                                        </td>
                                        <td class="flex gap-2">
                                            <a href="{{ route('deposit.page', ['deposit' => $deposit->id]) }}"
                                            ...
                                            <button @click="deletingId = {{ $deposit->id }}" command="show-modal" commandfor="dialog" type="button"
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="flex flex-col gap-2 p-6">
                    @forelse ($nowPaymentWallets as $np)
                        <a href="javascript:void(0)" wire:click="selectWallet(@js($np))" data-modal-hide="default-modal"
                            class="flex items-center justify-between gap-2 p-4 border border-neutral-200 dark:border-neutral-600 rounded-lg hover:bg-neutral-50 dark:hover:bg-neutral-600">
                            <span class="flex items-center gap-4">
                                <img src="{{ asset('assets/images/currency/' . strtolower($np['currency']) . '.png') }}"
                                    alt="" class="shrink-0 me-3 overflow-hidden">
                        ...
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                <h3 id="dialog-title" class="text-base font-semibold" style="color: black">
                                    Delete Deposit <span x-text="'d ' + deletingId + ' d'"></span>
                                </h3>
                                <div class="mt-2">
                                    <p class="text-sm" style="color: #292929">
                                        Are you sure you want to delete this deposit?
                                        The deposit will be permanently removed. This action cannot be undone.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                        <button commandfor="dialog" wire:click="deleteDeposit" type="button"
                            class="inline-flex w-full justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-red-500 sm:ml-3 sm:w-auto">
                            <span wire:loading.remove wire:target="deleteDeposit">Delete</span>
                            <x-btn-loader wire:loading wire:target="deleteDeposit" />
                        </button>
